            ...
            while current_player != -1:
                ...
                if is_draw_string:
                    ...
                    if 'm' in draw:
                        ...
                        continue
                # if drop_to[current_player] > len(log[drop_index]) - 1:
                #     drop = len(log[drop_index][-1:])
                ...
                if is_drop_string:
                    if 'a' in drop:
                        ...
                        if bonus_to < len(bonus_indicators):
                            indicator = Tile.his_to_34([bonus_indicators[bonus_to]])
                            bonus_to += 1
                            bonus_tiles.append(Tile.bns_ind_bd_dic.get(indicator[0], indicator[0] + 1))
                        continue
                    if 'k' in drop:
...
# def is_winning_hand(tiles, win_tile):
#     """
#     Finding the waiting tile via brute force with MahjongRepository Handscore
#     """
#     res = mahjong.hand_calculating(tiles, win_tile)
#     if()
# def calculate_waiting(tiles, win_tile):
#     tiles.append(win_tile)
#     mahjong.tile.TilesConverter.to_136_array(tiles)



def main():
    glc = GameLogCrawler()
    # glc.batch_crawl_refids(15, 20)
    # gene = glc.db_get_logs_where_players_lv_gr(19)
    # log = gene.__next__()
    # res = PreProcessing.process_one_log(log)
    # glc.batch_crawl_levels(100)
    # glc.batch_crawl_refids(19, 1000)
    # for i in range(1000):
    #     try:
    #         glc.batch_crawl_logs(19, 100)
    #     except Exception as e:
    #         print(e)
    #         pass
    glc.db_show_tables()
    ...
    for k,v in Errors.items():
        print("Error processing log {}:".format(k), v)
    print("Failed to Process {} logs".format(len(Errors)))
