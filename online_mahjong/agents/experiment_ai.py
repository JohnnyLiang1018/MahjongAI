...
class EnsembleCLF:
    ...
    def predict_normal_single_prio(self, input_f):
        ...
        for clf in self.clfs_normal:
            ...
            for p in predict:
                times[p] += 1
        return [[time / 120 for time in times[:34]], times[34] < 100]
    ...
    def predict_richii_single_prio(self, input_f):
        ...
        for clf in self.clfs_richii:
            ...
            for p in predict:
                times[p] += 1
        return [[time / 60 for time in times], True]
...
class HandAnalyser(HandParti):
    ...
    def deep_eff_vec(self, bot):
        ...
        while True:
            ...
            while current_index < len(normal_eff) and abs(normal_eff[index][1] - normal_eff[current_index][1]) < 4:
                current_index += 1
        ...
        for i in range(1, 4):
            ...
            if can_discard:
                if t34 in safe_tiles:
                    continue
                elif t34 in total_waiting or (near_bonus and len(total_waiting) > 0):
        ...
        if hand_ana.shantins[hand_ana.NORMAL] and hand_ana.shantins[hand_ana.NO19] <= 2:
            for tile in set(self.hand34):
                if tile in Tile.ONENINE and tile in self.game_table.bonus_tiles:
                    return False
            if self.cnt_total_bonus_tiles >= 2:
                return True
            if self.cnt_total_bonus_tiles >= 1 and self.turn_num >= 9:
                return True
        return False
