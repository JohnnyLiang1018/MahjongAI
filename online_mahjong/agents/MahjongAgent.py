...
class MahjongAgent(AIInterface):
    ...
    def yaku_goal_list(self,yaku_dict):
        ...
        for yaku in yaku_dict:
            ...
            for item in waiting_tile_list:
                if(type(item) is int):
                    print(item)
                    temp_p = 1 - (1 - ((4 - self.game_table.revealed_tiles[item])/self.game_table.count_remaining_tiles))**5
                    choice_list.append(temp_p)

                else:
                    temp_total = 0
                    for index in item:
                        temp_total += (4 -self.game_table.revealed_tiles[index])
                    temp_p = 1 - (1 - (temp_total/self.game_table.count_remaining_tiles))**5
                    choice_list.append(temp_p)
    ...
    # 3. For incomplete seq or single tile, calculate the possibility of some sort of advancing (for example, from single tile to seq-two-way, or from seq-one-way to seq-two-way etc). Give tiles weight based on that possibility. Then we have a list of tiles with weight, the one with the least weight should be the least important.
        partition = self.partition_dict(self.hand34,'seq')
        point_value = self.point_calculation(30,1)
        for tile in partition['seq-two-way']:
            total_remain = 8 - self.game_table.revealed_tiles[tile-1] - self.game_table.revealed_tiles[tile+2]
            prob_weight = total_remain / self.game_table.count_remaining_tiles * point_value
            if(tile in tile_weight_dict):
        ...
        for tile in partition['seq-one-way']:
            if(tile % 9 == 0):
                total_remain = 4 - self.game_table.revealed_tiles[tile+2]
            else:
                total_remain = 4 - self.game_table.revealed_tiles[tile-1]
            prob_weight = total_remain / self.game_table.count_remaining_tiles * point_value
            if(tile in tile_weight_dict):
                tile_weight_dict[tile] += prob_weight
            else:
        ...
        for tile in partition['seq-middle']:
            prob_weight = (4 - self.game_table.revealed_tiles[tile+1]) / self.game_table.count_remaining_tiles * point_value
            if(tile in tile_weight_dict):
                tile_weight_dict[tile] += prob_weight
            else:
                tile_weight_dict.setdefault(tile,prob_weight)
        ...
        for tile in partition['pair']:
            prob_weight = (4 - self.game_table.revealed_tiles[tile]) / self.game_table.count_remaining_tiles * point_value
            if(tile in tile_weight_dict):
                tile_weight_dict[tile] += prob_weight
            else:
                tile_weight_dict.setdefault(tile,prob_weight)
        ...
        for tile in partition['single']:
            prob = (4 - self.game_table.revealed_tiles[tile]) / self.game_table.count_remaining_tiles
            if(tile in tile_weight_dict):
    ...
    def to_discard_tile(self):
        ...
        partition_tri = self.partition_dict(self.hand34,'tri')
        partition_pair = self.partition_dict(self.hand34,'pair')
        yaku_dict = self.yaku_check(partition_seq,partition_tri,partition_pair,self.meld34)
