...
def run_example_ai(drawer=None):
    ...
    ai_module = importlib.import_module("agents.MahjongAgent")
    ai_class = getattr(ai_module, "MahjongAgent")
    ai_obj = ai_class()
    ...
    user = "ID76B06F13-c3mhaFM4"      # the user ID that you got after having registered in tenhou.net
    user_name = "weavile"      # the user name that you have created while registration in tenhou.net

    game_type = '137'      # '137' 南 '193' 东速高
...
def run_jianyang_ai(drawer=None):
    ai_module = importlib.import_module("agents.test_ai")
